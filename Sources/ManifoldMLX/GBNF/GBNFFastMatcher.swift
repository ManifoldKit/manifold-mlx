...
@_spi(Testing) public struct GBNFFastMatcher {
  ...
  public func acceptableFirstBytes() -> Set<UInt8> {
    var bytes: Set<UInt8> = []
    for s in stacks {
      guard let top = s.last, case .charset(let cs) = compiled.positions[top] else { continue }
      var b = 0
      while b <= 255 {
  ...
  static func step(_ input: Set<[Int]>, byte: UInt8, compiled c: GBNFCompiled) -> Set<[Int]> {
    var tails: [[Int]] = []
    for s in input {
      guard let top = s.last, case .charset(let cs) = c.positions[top], cs.contains(byte) else {
        continue
      }
      var t = s
      t[t.count - 1] = top + 1
