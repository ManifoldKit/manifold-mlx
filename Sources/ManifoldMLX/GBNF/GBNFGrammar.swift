...
@_spi(Testing) extension GBNFGrammar {
...
private struct GBNFParser {
  ...
  mutating func parse() throws -> GBNFGrammar {
    ...
    while i < tokens.count {
      guard case .ident(let name) = tokens[i] else {
    ...
    for alts in rules {
      for seq in alts {
        for sym in seq {
          if case .rule(let rid) = sym, rid >= rules.count {
  ...
  private mutating func parseAlternatives(_ c: inout GBNFTokenCursor) throws -> [[GBNFGrammar
    .Symbol]]
  {
