...
@_spi(Testing) public struct GBNFMatcher {
  ...
  public func acceptableFirstBytes() -> Set<UInt8> {
    var bytes: Set<UInt8> = []
    for s in stacks {
      guard let head = s.first, case .charset(let cs) = head else { continue }
  ...
  private func step(_ input: Set<[GBNFGrammar.Symbol]>, byte: UInt8) -> Set<[GBNFGrammar.Symbol]> {
    ...
    for s in input {
      guard let head = s.first, case .charset(let cs) = head, cs.contains(byte) else { continue }
