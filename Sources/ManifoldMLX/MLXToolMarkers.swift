...
@_spi(Testing) public enum MLXToolMarkers {
  ...
  @_spi(Testing) public static func toolCall(fromNative native: MLXLMCommon.ToolCall)
    -> ManifoldInference.ToolCall
  {
    ...
    return ManifoldInference.ToolCall(
      id: id, toolName: native.function.name, arguments: argumentsString)
  }
