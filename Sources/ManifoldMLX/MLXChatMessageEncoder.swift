...
@_spi(Testing) public enum MLXChatMessageEncoder {
  ...
  private static func toolFunctionsJSON(_ config: GenerationConfig, wrapInFunctionType: Bool)
    -> String
  {
  ...
  private static func toolAwareChatMessages(
    ...
    guard
      structuredHistory.contains(where: { message in
        message.parts.contains { part in
          if case .image = part { return true }
          return false
        }
    ...
    if toolAwareHistory.count < structuredHistory.count {
      for (offset, structuredMessage) in structuredHistory.dropFirst(toolAwareHistory.count)
        .enumerated()
      {
