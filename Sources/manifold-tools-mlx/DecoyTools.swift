...
enum DecoyTools {
  ...
  private static let pool: [Spec] = [
    Spec(
      name: "get_weather",
      description:
        "Returns the current weather for a city. Call when the user asks about weather, temperature, or forecast.",
      properties: [
        ("city", "City name, e.g. 'San Francisco'."), ("units", "'metric' or 'imperial'."),
      ]),
    Spec(
      name: "convert_currency",
      description: "Converts an amount between two currencies at the current exchange rate.",
      properties: [
        ("amount", "Amount to convert."), ("from", "Source ISO currency code."),
        ("to", "Target ISO currency code."),
      ]),
    Spec(
      name: "send_email",
      description:
        "Sends an email on the user's behalf. Call only when the user explicitly asks to send mail.",
      properties: [
        ("to", "Recipient address."), ("subject", "Subject line."), ("body", "Message body."),
      ]),
    Spec(
      name: "translate_text", description: "Translates text into a target language.",
      properties: [
        ("text", "Text to translate."), ("target_lang", "Target language code, e.g. 'es'."),
      ]),
    Spec(
      name: "set_timer", description: "Starts a countdown timer for a number of seconds.",
      properties: [("seconds", "Duration in seconds."), ("label", "Optional label for the timer.")]),
    Spec(
      name: "create_calendar_event", description: "Creates an event on the user's calendar.",
      properties: [
        ("title", "Event title."), ("start", "ISO-8601 start time."), ("end", "ISO-8601 end time."),
      ]),
    Spec(
      name: "get_stock_price", description: "Returns the latest trading price for a stock ticker.",
      properties: [("symbol", "Ticker symbol, e.g. 'AAPL'.")]),
    Spec(
      name: "play_music", description: "Plays a track from the user's music library.",
      properties: [("track", "Track name."), ("artist", "Artist name.")]),
    Spec(
      name: "send_sms", description: "Sends an SMS text message.",
      properties: [("phone", "Recipient phone number."), ("message", "Message text.")]),
    Spec(
      name: "book_flight", description: "Books a flight between two airports on a given date.",
      properties: [
        ("origin", "Origin airport code."), ("destination", "Destination airport code."),
        ("date", "Departure date."),
      ]),
    Spec(
      name: "get_directions", description: "Returns turn-by-turn directions between two locations.",
      properties: [("origin", "Start location."), ("destination", "End location.")]),
    Spec(
      name: "create_reminder", description: "Creates a reminder due at a given time.",
      properties: [("text", "Reminder text."), ("due", "ISO-8601 due time.")]),
    Spec(
      name: "post_social_update",
      description: "Posts a status update to the user's social account.",
      properties: [("message", "Status text.")]),
    Spec(
      name: "get_news_headlines", description: "Returns recent news headlines for a topic.",
      properties: [("topic", "Topic or keyword.")]),
    Spec(
      name: "control_smart_light", description: "Turns a smart light on or off in a given room.",
      properties: [("room", "Room name."), ("state", "'on' or 'off'.")]),
    Spec(
      name: "order_food", description: "Places a food delivery order from a restaurant.",
      properties: [("restaurant", "Restaurant name."), ("items", "Comma-separated items.")]),
    Spec(
      name: "start_workout", description: "Starts tracking a workout session.",
      properties: [
        ("type", "Workout type, e.g. 'run'."), ("duration", "Planned duration in minutes."),
      ]),
    Spec(
      name: "scan_qr_code", description: "Decodes a QR code from an image.",
      properties: [("image", "Path or identifier of the image.")]),
    Spec(
      name: "lookup_dictionary", description: "Returns the dictionary definition of a word.",
      properties: [("word", "Word to define.")]),
    Spec(
      name: "get_traffic", description: "Returns current traffic conditions along a route.",
      properties: [("route", "Route or road name.")]),
    Spec(
      name: "set_thermostat", description: "Sets the home thermostat to a target temperature.",
      properties: [("temperature", "Target temperature in degrees.")]),
    Spec(
      name: "find_parking", description: "Finds available parking near a location.",
      properties: [("location", "Location to search near.")]),
    Spec(
      name: "get_sunrise_sunset",
      description: "Returns sunrise and sunset times for a location and date.",
      properties: [("location", "Location name."), ("date", "Date in YYYY-MM-DD.")]),
    Spec(
      name: "track_package",
      description: "Returns the delivery status for a shipment tracking number.",
