...
struct CLI {
  ...
  static func defaultOutputURL() -> URL {
    ...
    return cwd.appendingPathComponent(
      "tmp/manifold-tools-mlx/\(TranscriptLogger.defaultFilename())")
  }
  ...
  static func parse(_ argv: [String]) -> CLI {
    ...
    guard commonOptions.extraTools <= DecoyTools.maxCount else {
      fail(
        "--extra-tools \(commonOptions.extraTools) exceeds the decoy pool size (\(DecoyTools.maxCount))"
      )
    }
...
func resolveCoreCommit(
  ...
  if let fromPackageResolved = packageResolved(), !fromPackageResolved.isEmpty {
    return fromPackageResolved
  }
  return "unknown"
}
  ...
  while i < argv.count {
    ...
    case "--core-commit":
      ...
      guard i < argv.count else {
        FileHandle.standardError.write(
          Data("record-identity: --core-commit requires a value\n".utf8))
        return 2
      }
...
func runCLI() async -> Int32 {
  ...
  guard let modelPath = cli.modelPath else {
    FileHandle.standardError.write(
      Data("manifold-tools-mlx: --model <path> is required (see --help)\n".utf8))
    return 2
  }
  ...
  if let marker = VLModelDetector.matchedMarkerFile(at: modelDirectory) {
    CLI.fail(
      "'\(modelPath)' looks like a vision-language model (found \(marker)); "
  ...
  if cli.extraTools > 0 {
    print(
      "Advertising \(cli.extraTools) decoy tool(s) per scenario: \(decoyNames.sorted().joined(separator: ", "))"
    )
  }
  ...
  let allPassed = await ScenarioCLIHarness.runAll(
    ...
    let registry = makeRegistry(
      for: scenario, fixturesRoot: fixturesRoot, extraTools: cli.extraTools)
    ...
    if !expected.isEmpty {
      ...
      print(
        "  tools: tp=\(counts.tp) fp=\(counts.fp) fn=\(counts.fn) (p=\(fmt3(counts.precision)) r=\(fmt3(counts.recall)) f1=\(fmt3(counts.f1)))"
      )
    }
  ...
  print(
    "SUMMARY extra_tools=\(cli.extraTools) passed=\(passedCount)/\(total) clean=\(cleanCount)/\(total) precision=\(fmt3(avgPrec)) recall=\(fmt3(avgRecall)) f1=\(fmt3(avgF1)) decoy_calls=\(decoyCallTotal) scored=\(toolBearingRecords.count)"
  )
