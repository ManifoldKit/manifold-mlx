...
open class FLUX {
  ...
  internal static func loadTokenizers(directory: URL, hub: HubApi) throws -> (
    ...
    let merges = try String(
      contentsOf: directory.appending(path: "tokenizer/merges.txt"), encoding: .utf8
    )
...
public class Flux1Schnell: FLUX, TextToImageGenerator, FLUXComponents, @unchecked Sendable {
  ...
  public static func createAndLoad(
    hub: HubApi, configuration: FluxConfiguration, dtype: DType = .float16
  ) throws -> Flux1Schnell {
...
extension FLUX {
  public func saveQuantizedWeights(to path: URL, bits: Int = 4, groupSize: Int = 64) throws {
  ...
  private func detectCurrentQuantization(_ fluxModel: FLUXComponents) -> (
  ...
  public func hasQuantizedLayers() -> Bool {
...
extension FLUX {
  public static func loadQuantized(
    ...
    if FileManager.default.fileExists(atPath: source) {
      ...
      let localURL = try await hub.snapshot(
        from: repo,
        matching: [
          "**/*.safetensors", "**/*.json", "**/*.txt", "metadata.json", "README.md",
          "**/spiece.model",
        ]
      ) { progress in
    ...
    let (loadedWeights, info) = try QuantizationUtils.loadQuantizedModel(
      from: URL(fileURLWithPath: modelPath))

    guard info.isQuantized,
      let bits = info.quantizationBits,
      let groupSize = info.groupSize
    else {
    ...
    if let loraPath = configuration.loraPath {
      ...
      do {
        let loraWeights = try model.loadLoraWeights(
          hub: hub, loraPath: loraPath, dType: configuration.dType)
  ...
  private static func createModel(type: String, hub: HubApi, modelDirectory: URL? = nil) throws
    -> FLUX
  {
...
public enum FluxError: LocalizedError {
  ...
  public var errorDescription: String? {
    ...
    case .quantizationMismatch(let current, let requested):
      return
        "Cannot re-quantize an already quantized model. Current: \(current), Requested: \(requested). Please load an unquantized model to apply different quantization parameters."
    }
  }
}
