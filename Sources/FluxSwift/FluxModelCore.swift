...
public class FluxModelCore: @unchecked Sendable {
  ...
  public init(
    hub: HubApi, fluxConfiguration: FluxConfiguration, modelConfiguration: FluxModelConfiguration
  ) throws {
  ...
  public init(hub: HubApi, modelDirectory: URL, modelConfiguration: FluxModelConfiguration) throws {
    ...
    (self.t5Tokenizer, self.clipTokenizer) = try FLUX.loadTokenizers(
      directory: modelDirectory, hub: hub)
  ...
  public func loadWeights(from directory: URL, dtype: DType = .float16) throws {
    ...
    anyQuantized =
      ...
      || anyQuantized
    anyQuantized =
      try loadVAEWeights(from: directory.appending(path: "vae"), dtype: dtype) || anyQuantized
    anyQuantized =
  ...
  private func loadTransformerWeights(from directory: URL, dtype: DType) throws -> Bool {
    ...
    let isQuantized =
      quantizationConfig(in: directory) != nil
      || transformerWeights.keys.contains { $0.hasSuffix(".scales") }
    if isQuantized {
      let (groupSize, bits) =
        quantizationConfig(in: directory)
        ?? (QuantizationUtils.defaultGroupSize, QuantizationUtils.defaultBits)
      applyPreQuantization(
        to: transformer, weights: transformerWeights, groupSize: groupSize, bits: bits)
    }
  ...
  public func conditionText(prompt: String) -> (MLXArray, MLXArray) {
    ...
    let paddedT5Tokens =
      Array(t5Tokens.prefix(configuration.t5MaxSequenceLength))
      + Array(
        repeating: 0,
        count: max(
          0,
          configuration.t5MaxSequenceLength - min(t5Tokens.count, configuration.t5MaxSequenceLength)
        ))

    let clipTokens = clipTokenizer.tokenize(text: prompt)
    let paddedClipTokens =
      Array(clipTokens.prefix(configuration.clipMaxSequenceLength))
      + Array(
        repeating: configuration.clipPaddingToken,
        count: max(
          0,
          configuration.clipMaxSequenceLength
            - min(clipTokens.count, configuration.clipMaxSequenceLength)))
