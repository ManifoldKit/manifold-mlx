...
public enum QuantizationUtils {
  ...
  public static func isQuantizedWeight(key: String) -> Bool {
    return key.hasSuffix(".scales") || key.hasSuffix(".biases")
...
extension QuantizationUtils {
  ...
  public static func saveQuantizedModel(
    ...
    if let modelDirectory = modelDirectory {
      ...
      logger.warning(
        "The quantized model will require internet access or cached tokenizers to work")
    }
  ...
  private static func saveModelComponent(
    ...
    for (index, chunk) in chunks.enumerated() {
      let chunkName = generateChunkName(
        for: chunk, index: index, totalChunks: chunks.count, componentName: name)
  ...
  private static func generateChunkName(
    for chunk: [(String, MLXArray)], index: Int, totalChunks: Int, componentName: String
  ) -> String {
    if totalChunks == 1 {
  ...
  public static func detectQuantization(at path: URL) -> QuantizedWeightInfo? {
    ...
    if FileManager.default.fileExists(atPath: metadataPath.path) {
      ...
      do {
        ...
        logger.warning(
          "Failed to load quantization metadata from \(metadataPath.lastPathComponent): \(error.localizedDescription)"
        )
      }
    }
