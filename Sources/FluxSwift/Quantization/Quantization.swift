...
public enum Quantization {
  ...
  public static func save(
    ...
    try QuantizationUtils.saveQuantizedModel(
      model, to: path, quantizationBits: bits, groupSize: groupSize)
  }
    ...
    for component in model.components {
      ...
      for (_, array) in params {
        if array.size > 1000 {
          ...
          let scalesSize =
            (array.size / QuantizationUtils.defaultGroupSize) * MemoryLayout<Float32>.size
...
extension FLUX {
  ...
  public var quantizationStats: QuantizationStats? {
    guard let fluxModel = self as? FLUXComponents else {
      return nil
    }
