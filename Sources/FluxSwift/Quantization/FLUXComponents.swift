...
extension FLUXComponents {
  ...
  public func component(named name: String) -> Module? {
    switch name {
    case "transformer":
      return transformer
    case "vae":
  ...
  public func updateComponentWeights(from weights: [String: MLXArray]) {
    let componentWeights = ComponentWeights.split(weights)

    if !componentWeights.vae.isEmpty {
      logger.debug(
        "Updating VAE weights",
        metadata: ["parameterCount": .string(String(componentWeights.vae.count))])
      vae.update(parameters: ModuleParameters.unflattened(componentWeights.vae))
    }

    if !componentWeights.transformer.isEmpty {
      logger.debug(
        "Updating transformer weights",
        metadata: ["parameterCount": .string(String(componentWeights.transformer.count))])
    ...
    if !componentWeights.clipEncoder.isEmpty {
      logger.debug(
        "Updating CLIP encoder weights",
        metadata: ["parameterCount": .string(String(componentWeights.clipEncoder.count))])
    ...
    if !componentWeights.t5Encoder.isEmpty {
      logger.debug(
        "Updating T5 encoder weights",
        metadata: ["parameterCount": .string(String(componentWeights.t5Encoder.count))])
      t5Encoder.update(parameters: ModuleParameters.unflattened(componentWeights.t5Encoder))
    }
  }

  public func applySelectiveQuantization(weights: [String: MLXArray], bits: Int, groupSize: Int) {
    ...
    for componentInfo in components {
      ...
      if quantizedCount > 0 {
        logger.debug(
          "Found pre-quantized layers",
          metadata: [
            "count": .string(String(quantizedCount)), "component": .string(componentInfo.name),
          ])
        ...
        logger.debug(
          "No pre-quantized weights found", metadata: ["component": .string(componentInfo.name)])
      }
    }
  }
}
