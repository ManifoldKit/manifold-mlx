...
func runReplay(hash: String, force: Bool, outputDir: URL, factory: any FuzzBackendFactory) async
  -> Int32
{
  ...
  case .reproduced(let r):
    ...
    print(
      "replay \(hash): reproduced \(r.successfulReproductions)/\(r.attempts) — \(verdict)\(drift)")
    return 0
  case .driftRefused(let report):
    print(
      "replay \(hash): drift refused (git \(report.recordedGitRev) → \(report.currentGitRev)); pass --force to override"
    )
    return 2
  case .recordNotFound:
...
while i < args.count {
  ...
  case "--seed":
    seed = UInt64(value()) ?? { fail("--seed requires an unsigned integer") }()
    seedSet = true
