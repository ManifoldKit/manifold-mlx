...
final class MLXKVPersistenceIntegrationTests: XCTestCase {
  ...
  private let followUpPrompt =
    "Using the earlier context only, answer with exactly the single word READY."
  ...
  private func runSecondTurnSample(
    ...
    XCTAssertFalse(
      firstTurn.text.isEmpty, "Turn 1 must produce text so the follow-up history is well formed")
  ...
  private func waitForIdle(_ backend: MLXBackend, timeoutSeconds: Int = 5) async throws {
    ...
    XCTAssertFalse(
      backend.isGenerating, "Backend should have stopped generating before the next turn begins")
  }
