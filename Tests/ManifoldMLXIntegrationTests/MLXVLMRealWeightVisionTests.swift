...
final class MLXVLMRealWeightVisionTests: XCTestCase {
  ...
  private static func configDeclaresSkipVision(at url: URL) -> Bool {
    ...
    guard let data = try? Data(contentsOf: configURL),
      ...
      let visionConfig = json["vision_config"] as? [String: Any]
    else {
      return false
    }
  ...
  private struct RunTurnTimeoutError: Error, CustomStringConvertible {
    ...
    var description: String {
      "runTurn timed out after \(seconds)s — possible VLM stream hang (#26)"
    }
  }
  ...
  func test_realVLM_sentSyntheticImage_respondsWithImageGroundedContent() async throws {
    ...
    let blindEvents = try await runTurn(
      on: backend, prompt: Self.visionQuestion, history: blindHistory)
    let blindText = collectAssistantText(from: blindEvents)
    print(
      "[MLXVLMRealWeightVisionTests] (blind control, no image) model=\(modelURL.lastPathComponent) reply=\"\(blindText)\""
    )
    ...
    let events = try await runTurn(
      on: backend, prompt: Self.visionQuestion, history: groundedHistory)
    ...
    print(
      "[MLXVLMRealWeightVisionTests] (grounded) model=\(modelURL.lastPathComponent) chosenColor=\(chosen.name) reply=\"\(text)\""
    )

    XCTAssertFalse(
      text.isEmpty,
      "A real vision-capable model must produce a non-empty reply to an image question.")
