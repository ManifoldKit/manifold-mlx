...
final class MLXBackendBenchmark: XCTestCase {
  ...
  func test_throughput() async throws {
    ...
    let warmup = try backend.generate(
      prompt: mlxBenchPrompt, systemPrompt: nil, config: config, hints: GenerationRuntimeHints())
...
private func printMLXResults(
  model: String, results: [(ttftMs: Double, totalMs: Double, tokens: Int)]
) {
  for (i, r) in results.enumerated() {
    let tps = Double(r.tokens) / (r.totalMs / 1000)
    print(
      String(
        format: "  [MLX run %d] TTFT=%.0fms  total=%.0fms  tokens=%d  TPS=%.1f",
  ...
  print(
    String(
      format: "MLX summary median TTFT=%.0fms median TPS=%.1f model=%@",
