...
final class MLXBackendGenerationTests: XCTestCase {
  ...
  func test_generate_withVisionHistory_preparesStructuredChat() async throws {
    ...
    XCTAssertNil(
      mock.lastMessages,
      "Vision turns should go through UserInput(chat:) instead of text-only message dictionaries")
  }
        ...
        hints: GenerationRuntimeHints(history: [
          ...
          StructuredMessage(
            ...
            parts: [
              .toolCall(
                ToolCall(
                  id: "call_1", toolName: "calc", arguments: "{\"a\":7823,\"op\":\"*\",\"b\":41}"))
            ]),
          StructuredMessage(
            role: "tool",
            parts: [
  ...
  func test_capabilities_kvCachePersistence_vlmRoutedModel_agreesWithNoReuse() async throws {
    ...
    XCTAssertEqual(
      mock.lastInitialCacheOffsets, [0],
  ...
  func test_generate_unsupportedCacheShapeBypassesReuseAndClearsSnapshot() async throws {
    ...
    let thirdEvents = try await collectAllEvents(
      from: try backend.generate(
        prompt: "third",
        systemPrompt: nil,
        config: GenerationConfig()
  ...
  func test_toolCall_extractedFromStream_wholeCall() async throws {
    ...
    XCTAssertFalse(
      visibleTokens.joined().contains("<tool_call>"),
  ...
  func test_generate_mistral_threadsStructuralTools() async throws {
    ...
    let stream = try backend.generate(
      prompt: "weather?", systemPrompt: nil, config: config, hints: GenerationRuntimeHints())
  ...
  func test_generate_llama_threadsNoStructuralTools() async throws {
    ...
    let threaded = try XCTUnwrap(
      mock.lastTools, "prepare(messages:tools:) is always the path taken")
  ...
  func test_toolCall_markerFragmentedAcrossChunks_reassembled() async throws {
    ...
    XCTAssertFalse(
      visibleTokens.joined().contains("<tool"),
      "Fragmented tool-call marker bytes must not leak into visible tokens")
    XCTAssertFalse(
      visibleTokens.joined().contains("_call>"),
    ...
    XCTAssertTrue(
      extractedArgs.first?.contains("Paris") == true,
  ...
  func test_chatTemplate_missingTemplate_propagatesApplyErrorUnchanged() async throws {
    ...
    XCTAssertTrue(
      unwrapped is MissingChatTemplateError,
  ...
  func test_chatTemplate_noAssistantMarker_propagatesApplyErrorUnchanged() async throws {
    ...
    XCTAssertTrue(
      unwrapped is NoAssistantMarkerError,
  ...
  func test_generate_forwardsNativeToolCallEvent() async throws {
    ...
    var config = GenerationConfig()
    config.tools = [
      ToolDefinition(name: "calc", description: "calculator", parameters: .object([:]))
    ]
  ...
  func test_generate_warnsWhenRunningContextExceedsTrainedMaximum() async throws {
    ...
    XCTAssertEqual(
      trainedCapture.calls.count, 1,
      "the primary warning must fire exactly once when the running context (11) exceeds the model's trained maximum (10)"
    )
  ...
  func test_generate_doesNotWarnWhenRunningContextStaysWithinTrainedMaximum() async throws {
    ...
    XCTAssertTrue(
      trainedCapture.calls.isEmpty,
      "a running context under the trained maximum must never trigger the primary warning")
  }
  ...
  func test_generate_warnsOnBudgetOnly_whenTrainedMaximumNotExceeded() async throws {
    ...
    XCTAssertTrue(
      trainedCapture.calls.isEmpty,
      "the running context (10) is nowhere near the trained maximum (1000) — the primary warning must not fire"
    )
    XCTAssertEqual(
      budgetCapture.calls.count, 1,
      "the running context (10) exceeds the budget (8) — the secondary warning must fire")
  ...
  func test_generate_noManifest_neverWarnsTrainedMaximum() async throws {
    ...
    XCTAssertTrue(
      trainedCapture.calls.isEmpty,
      "no injected manifest (nil trained ceiling) must never warn, regardless of running context size"
    )
  }
  ...
  func test_generate_trainedMaximumWarning_firesOnlyOncePerSession() async throws {
    ...
    XCTAssertEqual(
      trainedCapture.calls.count, 1,
      "three consecutive over-ceiling turns in the same session must log the primary warning exactly once, not once per turn"
    )
  }
  ...
  func test_generate_trainedMaximumWarning_rearmsAfterResetConversation() async throws {
    ...
    XCTAssertEqual(
      trainedCapture.calls.count, 2,
      "resetConversation() must re-arm the once-per-session warning for the new session")
  }
  ...
  func test_generate_warnsEvenWhenGenerationThrows() async throws {
    ...
    XCTAssertEqual(
      trainedCapture.calls.count, 1,
      "a throwing generation must still report the running context that had already exceeded the trained maximum before the failure"
    )
  ...
  func test_generate_doesNotWarnPrimary_whenTrainedContextWasNotDetected() async throws {
    ...
    XCTAssertTrue(
      trainedCapture.calls.isEmpty,
      "a nil contextWindow must never drive the primary trained-context warning, even when the running context (~1,000,003) is enormous — there is no measured number to compare against"
    )
  ...
  func test_generate_stillWarnsBudget_whenTrainedContextWasNotDetected() async throws {
    ...
    XCTAssertTrue(
      trainedCapture.calls.isEmpty,
      "the fixture's contextWindow is nil, so the primary warning correctly stays silent even at a running context of ~1,000,003"
    )
    XCTAssertEqual(
      budgetCapture.calls.count, 1,
      "the budget warning is independent of detection provenance and must still fire when running (~1,000,003) exceeds the budget (5)"
    )
  }
  ...
  func test_reinject_resetsRateLimitedWarningFlags() async throws {
    ...
    XCTAssertEqual(
      trainedCapture.calls.count, 1,
      "precondition: first over-ceiling turn on the first injected container warns")
    ...
    XCTAssertEqual(
      trainedCapture.calls.count, 2,
      "_inject(...) must reset the once-per-session warning flags — a re-injected backend must not silently inherit a suppressed warning from the previous container"
    )
