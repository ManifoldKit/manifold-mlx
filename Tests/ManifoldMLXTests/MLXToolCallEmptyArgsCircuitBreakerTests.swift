...
final class MLXToolCallEmptyArgsCircuitBreakerTests: XCTestCase {
  ...
  func test_nonEmptyArgs_qwen_parsesCorrectly() throws {
    ...
    let body =
      #"<tool_call>{"name":"search","arguments":{"query":"swift async","limit":5}}</tool_call>"#
  ...
  func test_legitimateEmptyArgs_qwen_passesThrough() throws {
    ...
    let calls = toolCalls(events)
    XCTAssertEqual(
      calls.count, 1, "a tool call with legitimately empty arguments must not be dropped")
  ...
  func test_preSerializedEmptyStringArgs_passesThrough() throws {
    ...
    let calls = toolCalls(events)
    XCTAssertEqual(
      calls.count, 1, "pre-serialised empty string args must pass through (guard must not fire)")
  ...
  func test_circuitBreaker_doesNotFire_whenNoArgsKeyInBody() throws {
    ...
    let calls = toolCalls(events)
    XCTAssertEqual(
      calls.count, 1, "a name-only body with no args key must pass through as empty-args call")
