...
final class MLXMistralToolDialectTests: XCTestCase {
  ...
  func test_singleCall_parsesCorrectly() throws {
    ...
    var events = parser.process(
      #"[TOOL_CALLS] [{"name": "get_weather", "arguments": {"location": "Boston, MA"}}]"#)
  ...
  func test_mistralProseToolBlock_isGatedOff() {
    ...
    XCTAssertNil(
      MLXChatMessageEncoder.buildQwenToolBlock(
        config: config(withTools: [weatherTool()]), dialect: .mistral),
  ...
  func test_mistralStructuralToolSpecs_areThreaded() throws {
    let specs = try XCTUnwrap(
      MLXChatMessageEncoder.structuralToolSpecs(
        config: config(withTools: [weatherTool()]), dialect: .mistral),
  ...
  func test_structuralToolSpecs_nilForProseDialects() {
    ...
    XCTAssertNil(
      MLXChatMessageEncoder.structuralToolSpecs(
        config: config(withTools: [weatherTool()]), dialect: .llama))
    XCTAssertNil(
      MLXChatMessageEncoder.structuralToolSpecs(
        config: config(withTools: [weatherTool()]), dialect: .qwen25))
    XCTAssertNil(
      MLXChatMessageEncoder.structuralToolSpecs(
        config: config(withTools: [weatherTool()]), dialect: .unknown))
  }

  func test_structuralToolSpecs_nilWhenNoTools() {
    XCTAssertNil(
      MLXChatMessageEncoder.structuralToolSpecs(config: config(withTools: []), dialect: .mistral))
  }

  func test_mistralProseToolBlock_nilWhenNoTools() {
    XCTAssertNil(
      MLXChatMessageEncoder.buildQwenToolBlock(config: config(withTools: []), dialect: .mistral))
  }
  ...
  func test_effectiveSystemPrompt_mistral_containsPreferToolsPreamble() throws {
    ...
    XCTAssertFalse(
      output.contains("Available functions:"),
  ...
  func test_missingName_elementDropped_siblingsStillParse() throws {
    ...
    XCTAssertEqual(
      toolCalls(events).map(\.toolName), ["ok"],
