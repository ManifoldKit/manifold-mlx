...
final class MLXBackendTests: XCTestCase {
  ...
  func test_generate_beforeLoad_throws() {
    XCTAssertThrowsError(
      try MLXBackend().generate(
        prompt: "hi", systemPrompt: nil, config: GenerationConfig(), hints: GenerationRuntimeHints()
      )
    )
  }
  ...
  func test_generate_withSupportedGrammar_isAcceptedNotRejected() {
    ...
    XCTAssertThrowsError(
      try MLXBackend().generate(
        prompt: "hi", systemPrompt: nil, config: config, hints: GenerationRuntimeHints())
    ) { error in
  ...
  func test_loadModel_invalidDirectory_throws() async throws {
    ...
    do {
      ...
      guard case InferenceError.modelLoadFailed = error else {
        return XCTFail(
          "Expected InferenceError.modelLoadFailed for a nonexistent model directory, got \(error)")
      }
      XCTAssertFalse(
        b.isModelLoaded,
