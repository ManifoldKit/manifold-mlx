...
final class MLXLlamaToolDialectTests: XCTestCase {
  ...
  func test_realLlamaCalcCall_dispatches() throws {
    ...
    XCTAssertEqual(
      calls.count, 1, "the captured Llama calc call must parse to exactly one tool call")
    let call = try XCTUnwrap(calls.first)
    XCTAssertEqual(call.toolName, "calc")
    let args = try XCTUnwrap(
      try JSONSerialization.jsonObject(with: XCTUnwrap(call.arguments.data(using: .utf8)))
        as? [String: Any]
    )
  ...
  func test_parametersKey_isParsedAsArguments() throws {
    ...
    let events = parser.process(
      #"<tool_call>{"name":"read_file","parameters":{"path":"a.txt"}}</tool_call>"#)
  ...
  func test_trailingTailAfterJSON_isIgnored() throws {
    var parser = Parser(dialect: .llama)
    let events = parser.process(
      #"<tool_call>{"name":"calc","parameters":{"a":1,"op":"+","b":2}} <|eot_id|></tool_call>"#)
  ...
  func test_llamaToolBlock_isInjectedWithToolGrammar() throws {
    let block = try XCTUnwrap(
      MLXChatMessageEncoder.buildQwenToolBlock(
        config: config(withTools: [calcTool()]), dialect: .llama),
    ...
    XCTAssertTrue(
      block.contains("\"calc\"") || block.contains("calc"), "must list the available function")
  }

  func test_noToolBlock_whenNoTools() {
    XCTAssertNil(
      MLXChatMessageEncoder.buildQwenToolBlock(config: config(withTools: []), dialect: .llama))
  }

  func test_unknownDialect_getsNoToolBlock() {
    XCTAssertNil(
      MLXChatMessageEncoder.buildQwenToolBlock(
        config: config(withTools: [calcTool()]), dialect: .unknown))
  }
  ...
  func test_llamaHistoryEncoding_usesToolCallWrapperWithParameters() throws {
    ...
    XCTAssertTrue(
      content.contains("\"parameters\""), "replayed Llama calls use the `parameters` key")
  ...
  func test_pythonTag_withoutVisibleClose_dispatchesAfterNormalisation() throws {
    ...
    var events = parser.process(
      #"<|python_tag|>{"name":"read_file","parameters":{"path":"a.txt"}}"#)
    events += parser.finalize()
    let calls = toolCalls(events)
    XCTAssertEqual(
      calls.map(\.toolName), ["read_file"],
  ...
  func test_pythonTag_withVisibleEomClose_dispatchesOnce() throws {
    ...
    events += parser.finalize()
    XCTAssertEqual(
      toolCalls(events).map(\.toolName), ["now"],
  ...
  func test_effectiveSystemPrompt_containsPreferToolsPreamble_forTools() throws {
    ...
    let output = try XCTUnwrap(
      result, "effectiveSystemPrompt must not be nil when tools are present")
    XCTAssertTrue(
