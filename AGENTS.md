...
- This package pins ManifoldKit via `.upToNextMinor(from: "0.65.0")` (traits: `[]`). `core-bump.yml` auto-bumps that pin on every ManifoldKit release, gates on a full rebuild+retest, and admin-merges the resulting `fix:` PR.
